...
export async function PUT(request, { params }) {
  try {
    ...
    if (action === "approve") {
      // Check if apartment ownership already exists
      const [existingOwnership] = await db
        .select()
        .from(apartmentOwnerships)
        .where(
          and(
            eq(apartmentOwnerships.userId, apartmentRequest.userId),
            eq(apartmentOwnerships.apartmentId, apartmentRequest.apartmentId),
          ),
        )
        .limit(1);

      if (existingOwnership) {
        // Update existing ownership to approved
        await db
          ...
          .where(eq(apartmentOwnerships.id, existingOwnership.id));
      } else {
      ...
      for (const member of requestMembers) {
        ...
        if (member.mobileNumber) {
          ...
          const [newUser] = await db
            ...
            .values({
              name: member.name,
              mobileNumber: `t_${Math.random().toString(36).substring(2, 10)}`,
              email: null,
              password: null,
              mobileVerified: false,
              emailVerified: false,
