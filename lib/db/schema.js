...
export const rentSessions = mysqlTable('rent_sessions', {
  ...
  status: mysqlEnum('status', ['pending_tenant_approval', 'active', 'completed', 'terminated']).default('pending_tenant_approval'),
...
export const disputeChatMessages = mysqlTable('dispute_chat_messages', {
  ...
  senderRole: mysqlEnum('sender_role', ['owner', 'tenant', 'admin', 'system']).notNull(),
...
export const classifieds = mysqlTable('classifieds', {
  ...
  condition: mysqlEnum('condition', ['excellent', 'good', 'fair']),
  isNegotiable: boolean('is_negotiable').default(true),
  status: mysqlEnum('status', ['available', 'sold', 'reserved']).default('available'),
...
// 43. Community Post Comment Likes
export const communityPostCommentLikes = mysqlTable('community_post_comment_likes', {
  id: bigint('id', { mode: 'number', unsigned: true }).primaryKey().autoincrement(),
...
// 44. Classified Likes
export const classifiedLikes = mysqlTable('classified_likes', {
  id: bigint('id', { mode: 'number', unsigned: true }).primaryKey().autoincrement(),
  classifiedId: bigint('classified_id', { mode: 'number', unsigned: true }).notNull(),
  userId: bigint('user_id', { mode: 'number', unsigned: true }).notNull(),
  createdAt: timestamp('created_at').defaultNow(),
